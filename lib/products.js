...
export function getProducts({ category = "all", q = "", sort = "popular", availability = "all", page = 1, pageSize = 24 } = {}) {
  ...
  const rows = db
    ...
    .all({ ...values, $limit: Number(pageSize), $offset: offset });

  return rows.map(mapProduct);
}

export function countProducts({ category = "all", q = "", availability = "all" } = {}) {
  const conditions = ["p.is_available = 1"];
  const values = {};
  if (category !== "all") {
    conditions.push("c.slug = $category");
  ...
  if (q) {
    conditions.push("(LOWER(p.name) LIKE $query OR LOWER(p.description) LIKE $query OR LOWER(p.short_description) LIKE $query)");
    values.$query = `%${q.toLowerCase()}%`;
  }
  if (availability === "in-stock") conditions.push("p.stock_quantity > 0");
  if (availability === "out-of-stock") conditions.push("p.stock_quantity <= 0");
  return db
    .prepare(
      `SELECT COUNT(*) AS count FROM products p JOIN categories c ON c.id = p.category_id WHERE ${conditions.join(" AND ")}`
    )
    .get(values).count;
}
